...
from skills.skill_trend_research import TrendResearchInput, TrendResearchOutput, execute_trend_research
...
def test_all_skills_have_input_output_models():
    ...
    for module_name, input_model, output_model in skill_modules:
        ...
        assert issubclass(input_model, BaseModel), f"{module_name} Input must be Pydantic model"
        assert issubclass(output_model, BaseModel), f"{module_name} Output must be Pydantic model"
...
def test_all_skills_have_execute_function():
    ...
    for module_name, execute_func in skill_functions:
        ...
        assert execute_func.__name__ == f"execute_{module_name.replace('skill_', '')}", \
...
def test_skill_output_includes_confidence_score():
    ...
    for model in output_models:
        assert "confidence_score" in model.model_fields, \
